...
/**
 * $Id: Output.php 583 2019-02-07 13:35:54Z dab $
 *
        ...
        if ($debug || $html) {
            ...
            if ($debug) {
                ...
                if (preg_match("/schemaLocation=\".+\s([a-zA-Z0-9_\/\.\-]+)\"/", $data, $matches)) {
                    $schema = ($documentURI ? dirname($documentURI) . "/" : "") . $matches[1];
                    if (!file_exists($schema)) {
                        trigger_error("schema '" . $schema . "' not found");
                    }
                    libxml_use_internal_errors(true);
                    if (!$outputDom->schemaValidate($schema)) {
                        $errors = libxml_get_errors();
                        libxml_use_internal_errors(false);
                        $out = "";
                        foreach ($errors as $error) {
                            if ($error->level == LIBXML_ERR_WARNING && $error->code == 3083) {
                                //ignore "Skipping import of schema ... since this namespace was already imported ..."
                                //continue;
                            }
                            switch ($error->level) {
                            ...
                            $out .= ":" . $error->line . ": " . $error->message . PHP_EOL;
                        }
                        if ($out) {
                            throw new Exception("DOMDocument::schemaValidate(): " . $out);
                        }
                    }
                    libxml_use_internal_errors(false);
//                } else {
//                    throw new Exception("cant find schemaLocation");
            ...
            $rootnodes = $outputDom->childNodes;
            $stylesheet = null;
            for( $i=0; $i < $rootnodes->length; $i++) {
                if ($rootnodes->item($i)->nodeType == XML_PI_NODE &&
                    $rootnodes->item($i)->target == "xml-stylesheet") {
                       $stylesheet = $rootnodes->item($i)->data;
                       break;
                }
            }
            unset($rootnodes);
            $matches = null;
            if ($stylesheet && preg_match("/href\s*=\s*\"([^\"]+)\"/", $stylesheet, $matches)) {
                ...
                //на время разбора шаблона ставим свой специальный обработчик ошибок
        ...
        if ($debug && $xsltResult) {
            ...
            $xsltResultDoc->loadXML($xsltResult);
            if ($documentURI) {
                $outputDom->documentURI = $documentURI;
            }
            //проверим что использован xsl:output с указанным doctype
            if (!$xsltResultDoc->doctype) {
                    throw new Exception("xsl:output/doctype not found in template");
            }
            $html5output = false;
            ...
            if (!$html5output) { //для HTML5 схем не завезли и не завезут никогда...
            ...
            $xsltResultDoc = null;
            unset($xsltResultDoc);
            if (!$html5output) { //tidy плохо знает html5
                ...
                //уберемся за собой
                unset($tidy, $config);
            } else { //html5 проверяем через локально развернутый сервис validator.nu
                $ch = curl_init();
                ...
                curl_setopt($ch, CURLOPT_USERAGENT, "XML_Output::tryHTML");
                ...
                $out = preg_replace("/^:.+info warning: The “date” input type is so far supported properly.*/m", "", $out);
                $out = preg_replace('/^:.+info warning: Documents should not use “about:legacy-compat”.*/m', "", $out);
                if (trim($out)) {
                    $xsltResultLines = explode(PHP_EOL, $xsltResult);
                    $xsltResultWithLines = "";
                    foreach ($xsltResultLines as $lineNumber => $line) {
            ...
            header("X-XML-Output-tryHTML: HTML=" . ($html ? $html : "false") . " XSLTtime=" . sprintf("%0.4f", $xsltStop - $xsltStart) . "s size=" . mb_strlen($data, "ASCII") . "/" . mb_strlen($xsltResult, "ASCII"));
        }
        ...
        if ($html) {
            unset($data);
            //тем кто заявил что понимает application/xhtml+xml отдаем xhtml с правильным типом
            if ($htmlContentType == "text/html" && isset($_SERVER["HTTP_ACCEPT"]) && strpos($_SERVER["HTTP_ACCEPT"], "application/xhtml+xml") !== false && strpos(substr($xsltResult, 30, 20), "XHTML") !== false) {
