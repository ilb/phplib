...
/**
 * $Id: Request2Xml.php 615 2019-11-26 16:27:58Z dab $
 *
...
class HTTP_Request2Xml {
    ...
    /**
     * разделитель элементов в идентификаторах элементов формы ( вместо XPath-ового "/" )
     */
     ...
     */
    public $empty = TRUE;
    public static $default_replace_pairs = array(
     ...
     */
    public function __construct($schema, $request = NULL, $root = NULL, $replace_pairs = NULL) {
        ...
        if (!$request &&
            ...
            if (strpos($GLOBALS["HTTP_RAW_POST_DATA"], "<!DOCTYPE") !== false ) { //antiXXE
                throw new Exception("DOCTYPE declaration not allowed", 450);
            }
            $oldValue = libxml_disable_entity_loader(true); //antiXXE
            //не tread-safe - явно восстанавливаем значение для всех других скриптов в данном php-процессе
            register_shutdown_function(function($oldValue) { libxml_disable_entity_loader($oldValue); }, $oldValue);
            //пытаемся грузить хмл как есть в дом
            $this->dom->loadXML($GLOBALS["HTTP_RAW_POST_DATA"]);
            libxml_disable_entity_loader($oldValue); //antiXXE
            ...
            if (!$request) {
                ...
                if (array_key_exists("<?xml_version", $request)) {
                    throw new Exception("invalid mime-type: XML must have 'application/xml' type", 450);
                }
            }
            ...
            if (count($request)) {
                ...
                $xr = new XMLReader();
                $xr->xml(file_get_contents($this->schema));
                $this->readNSinfo($xr);
                if ($this->root === NULL) {
                            ...
                            if ($j < $len - 1) {
                                ...
                                if (is_array($val)) {
                                    ...
                                    if (array_key_exists($key, $_FILES)) {
                                        ...
                                        for ($a = 0; $a < $count; $a++) {
                                            $children[$path] = $parent->appendChild($this->dom->createElementNS($ns, $name));
                                            $children[$path]->appendChild($this->dom->createElementNS($ns, "name", self::cleanup($_FILES[$key]["name"][$a], $replace_pairs)));
                                        ...
                                        foreach ($val as $subval) {
                                            $subval = self::cleanup($subval, $replace_pairs);
                                    ...
                                    $val = self::cleanup($val, $replace_pairs);
                                    if ($nolastindex) {
     ...
     * @param string $name имя элемента
     * @return array результат из массива значений либо пустой
     */
    ...
    public function validate() {
        ...
        try {
            $res = $this->dom->schemaValidate($this->schema);
        } catch (Exception $e) {
     ...
     * @return string
     */
    public static function cleanup($value, $replace_pairs = NULL) {
        return trim(strtr($value, ($replace_pairs !== NULL ? $replace_pairs : static::$default_replace_pairs)));
    }
